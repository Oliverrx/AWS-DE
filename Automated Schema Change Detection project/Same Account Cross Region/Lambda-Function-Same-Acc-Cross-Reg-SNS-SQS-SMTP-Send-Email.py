...
def smtp_send_email(res, acc, reg, db, tb, ch):
    ...
    msg["Subject"] = "Same Account Cross Region Schema Change Detected"
    msg["From"] = send_from
    msg["To"] = send_to
    
    body_html = """\
    <html>
      <head><h1>Same Account Cross Region Schema Change Detected</h1></head>
      <body>
        <p>
            <br>
...
def send_to_sns(msg):
    ...
    snsArn = "arn:aws:sns:us-east-1:823650347705:send_Email_SNS_topic"
    
    response = client.publish(
        TopicArn = snsArn,
        Message = msg,
        Subject = "Same Account Cross Region Schema Change Detected"
        )
