...
def smtp_send_email(res, acc, reg, db, tb, ch):
   ...
   msg["Subject"] = "Same Account Same Region Schema Change Detected"
   msg["From"] = send_from
   msg["To"] = send_to
             
   body_html = """\
      <html>
      <head><h1>Same Account Same Region Schema Change Detected</h1></head>
      <body>
      <p>
         <br>
...
def send_to_sns(msg):
   ...
   response = client.publish(
      TopicArn = snsArn,
      Message = msg,
      Subject = "Same Account Same Region Schema Change Detected"
      )
...
def lambda_handler(event, context):
   ...
   for Record in event['Records']:
      ...
      # table = deserialized_message["detail"]["changedTables"]
...
# def smtp_send_email(res, acc, reg, db, tb, ntb, ch):
#     msg = MIMEMultipart("alternative")
#     msg["Subject"] = "Same Account Same Region Schema Change Detected"
#     msg["From"] = send_from
#     msg["To"] = send_to
...
#         database = deserialized_message["detail"]["databaseName"]
#         table = deserialized_message["detail"]["tableName"]
#         new_table = deserialized_message["detail"]["changedTables"]
#         change = deserialized_message["detail"]["typeOfChange"]
        
#         smtp_send_email(resource, account, region, database, table, new_table, change)
#         send_to_sns(change)
#待改     
